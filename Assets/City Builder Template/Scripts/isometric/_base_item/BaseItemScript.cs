...
public class BaseItemScript : MonoBehaviour
{
	...
	{
		Vector2 a = Utilities2.GetScreenPosition(this.transform.position - new Vector3(0, 0, 1));
		Vector2 b = Utilities2.GetScreenPosition(this.transform.position);
		Vector2 c = Utilities2.GetScreenPosition(point);

		float angle = Utilities2.ClockwiseAngleOf3Points(a, b, c);
		this.SetAngle(angle);
	}
